...
data class Square(val column: Int, val row: Int) {
    ...
    fun notation(): String {
        if (!inside())
            return ""
        return ("${(column + '`'.toInt()).toChar()}$row")
    }
}
...
fun square(notation: String): Square {
    ...
    return (Square(notation[0].toInt() - '`'.toInt(), notation[1].toInt() - '0'.toInt()))
}
...
fun kingMoveNumber(start: Square, end: Square): Int {
    ...
    var shift = start.row
    var counter = 0
    val a = end.row > start.row
    while (abs(end.row - shift) != abs(end.column - start.column)) {
        if (a)
            shift++
        else
            shift--
    ...
    return abs(end.column - start.column) + counter
}
